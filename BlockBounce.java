...
public class BlockBounce {
  ...
  private static ArrayList<Double> trueBlocksX = new ArrayList<Double>(); // keeps track of the x coordinate for
                                                                          // filled blocks
  private static ArrayList<Double> trueBlocksY = new ArrayList<Double>(); // keeps track of the y coordinate for
                                                                          // filled blocks
  ...
  private static void updateBallPosition() {
    ...
    if ((Math.abs(rx + vx) > xScale - radius))
      vx = -vx;
    if ((Math.abs(ry + vy) > yScale - radius))
      vy = -vy;
    // detects the block
  ...
  private static void detectBlock() { // Detects if a ball is near a box and manipulates the direction of the ball
                                      // upon detection
    boolean hitX = false;
    boolean hitY = false;
    for (int i = 0; i < trueBlocksX.size(); i++) {
      if ((((trueBlocksX.get(i) + 1.5 * radius) > (rx + vx)) && (trueBlocksX.get(i) < (rx + vx))) && // detects right
                                                                                                     // side

          ((trueBlocksY.get(i) + 1.5 * radius) > (ry + vy) && (trueBlocksY.get(i) - 1.5 * radius) < (ry + vy))) {
        hitX = true;
      } else if ((((trueBlocksX.get(i) - 1.5 * radius) < (rx + vx)) && (trueBlocksX.get(i) > (rx + vx))) && // detects
                                                                                                            // left side
          ((trueBlocksY.get(i) + 1.5 * radius) > (ry + vy) && (trueBlocksY.get(i) - 1.5 * radius) < (ry + vy))) {
        hitX = true;
      }
      if ((((trueBlocksY.get(i) + 1.5 * radius) > (ry + vy)) && (trueBlocksY.get(i) < (ry + vy))) && // detects top of
                                                                                                     // box
          ((trueBlocksX.get(i) + 1.5 * radius) > (rx + vx) && (trueBlocksX.get(i) - 1.5 * radius) < (rx + vx))) {
        hitY = true;
      } else if ((((trueBlocksY.get(i) - 1.5 * radius) < (ry + vy)) && (trueBlocksY.get(i) > (ry + vy))) && // detects
                                                                                                            // bottom of
                                                                                                            // box
          ((trueBlocksX.get(i) + 1.5 * radius) > (rx + vx) && (trueBlocksX.get(i) - 1.5 * radius) < (rx + vx))) {
        hitY = true;
    ...
    for (int i = 0; i < 20; i++) {
      for (int j = 0; j < 20; j++) {
        if (!grid[j][i]) {
          ...
          StdDraw.filledRectangle(2 * radius * (j - xScale + 2.5 * border), 2 * radius * (i - yScale + 2.5 * border),
              radius, radius);
          count++;
